/**
 * PKV-§204 Scraper — Welle 5 (erweiterte Keyword-Liste).
 *
 * Wie v4 (Serper.dev), aber breiter:
 *   - 18 Keywords statt 6
 *   - 30 Results pro Query (statt 20)
 *   - Erwartung: ~200-400 Domain-Kandidaten, ~50-100 nach Verify + Enrich
 *   - Verbraucht ~18 von 2500 Serper-Credits
 *
 * Setup:
 *   1. export SERPER_API_KEY="dein-key"
 *   2. npm run scrape:pkv-204-v5
 */
...
const OUTPUT_PATH = "data/pkv-204-v5.csv";
...
const KEYWORDS = [
  ...
  // Erweiterung — Marketing- und Alltagsvokabular
  "PKV Beitragsentlastung Beratung",
  "PKV günstiger Tarif Honorarberater",
  "PKV Beitragserhöhung Widerspruch Berater",
  "PKV Beitragssenkung Experte Deutschland",
  "PKV Sparpotenzial Tarifanalyse",
  "PKV alten Tarif prüfen Beratung",
  "PKV Umstellung Honorarberatung",
  "PKV Selbstbehalt Optimierung Anbieter",
  "PKV Tarifprüfung Deutschland",
  "PKV Basistarif Standardtarif Wechsel",
  "PKV Beitragsanpassung Optimierung",
  "PKV Beratung Beitragsersparnis Honorarbasis",
];
...
async function main() {
  ...
  if (!apiKey && !args.skipSerp) {
    ...
    console.error("  npm run scrape:pkv-204-v5");
    process.exit(1);
  }
  console.log(`PKV-204 v5 (Serper API / Google, ${KEYWORDS.length} Keywords) · concurrency=${args.concurrency} · count=${args.count}`);
  ...
  console.log(`  ✓ ${rows.length} Zeilen (${seedCount} Seed · ${rows.length - seedCount} Serper-neu über ${KEYWORDS.length} Keywords)`);
